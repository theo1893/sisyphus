...
class BrowserAutomation:
    def __init__(self):
        ...
        # Direct state access
        self.router.get("/automation/get_browser_state")(self.get_browser_state)
    ...
    async def startup(self):
        ...
        try:
            ...
            # Get existing pages from the browser context first
            ...
            if existing_pages:
                ...
                print("No existing pages, creating initial page")
                # Create initial page BEFORE setting up event handler
                page = await self.browser_context.new_page()
                self.pages = [page]
                self.current_page_index = 0
                # Navigate to google.com
                await page.goto("https://www.google.com", wait_until="domcontentloaded", timeout=30000)
                print("Created initial page and navigated to google.com")

            # Set up page event handler AFTER initial setup
            try:
                self.browser_context.on("page", self.handle_page_created)
                print("Page event handler set up")
    ...
    async def get_updated_browser_state(self, action_name: str) -> tuple:
        ...
        try:
            ...
            return dom_state, "", elements, metadata
        except Exception as e:
    ...
    async def click_coordinates(self, action: ClickCoordinatesAction = Body(...)):
        ...
        try:
            ...
            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state(
                f"click_coordinates({action.x}, {action.y})")
    ...
    async def click_element(self, action: ClickElementAction = Body(...)):
        ...
        try:
            ...
            if action.index not in selector_map:
                ...
                dom_state, screenshot, elements, metadata = await self.get_updated_browser_state(
                    f"click_element_error (index {action.index} not found)")
                return self.build_action_result(
                    False,
    ...
    async def input_text(self, action: InputTextAction = Body(...)):
        ...
        try:
            ...
            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state(
                f"input_text({action.index}, '{action.text}')")
    ...
    async def select_dropdown_option(self, index: int = Body(...), option_text: str = Body(...)):
        ...
        try:
            ...
            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state(
                f"select_dropdown_option({index}, '{option_text}')")
    ...
    async def drag_drop(self, action: DragDropAction = Body(...)):
        ...
        try:
            ...
            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state(
                f"drag_drop({action.element_source}, {action.element_target})")
    ...
    async def get_browser_state(self):
        """Direct API endpoint to get the current browser state
        This allows external services to retrieve the current state of the browser
        without performing any actions.
        """
        try:
            # Get current browser state
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state(
                "get_browser_state_api_call")

            return self.build_action_result(
                True,
                "Retrieved current browser state",
                dom_state,
                screenshot,
                elements,
                metadata,
...
async def test_browser_api():
    ...
    try:
        ...
        # Test direct browser state API
        print("\n--- Testing Direct Browser State API ---")
        state_result = await automation_service.get_browser_state()
        print(f"Browser state API status: {'✅ Success' if state_result.success else '❌ Failed'}")
        if state_result.success:
            print(f"State API URL: {state_result.url}")
            print(f"State API Title: {state_result.title}")
            print(f"State API Elements: {state_result.element_count}")
            print(f"State API Message: {state_result.message}")
        else:
            print(f"State API Error: {state_result.error}")
